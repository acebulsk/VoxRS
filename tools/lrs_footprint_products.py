...
def lrs_footprint_products(batch_dir, cn_coef, file_out=None, threshold=False):
    ...
    if file_out is True:
        if threshold:
            file_out = batch_dir + "rshmetalog_footprint_products_threshold" + str(threshold) + ".csv"
        else:
            file_out = batch_dir + "rshmetalog_footprint_products_alldeg.csv"
    ...
    for zz in range(0, z_count):
        ...
        for ii in tqdm(range(0, z_num), desc="img chunk" + str(zz + 1), ncols=100, leave=True):
            # cn_stack[:, :, ii] = tif.imread(batch_dir + "" + rshmeta.file_name[ii])[:, :, 0] * cn_coef
            im = tif.imread(batch_dir + "" + rshmeta.file_name[zz * z_step + ii])
